...
export class OrgDispatchService {
  ...
  getBoard(orgId: string, status?: string, page = 0, size = 30) {
    ...
    return this.http.get<{ data: DispatchItem[]; length: number }>(
  ...
  getStats(orgId: string) {
    return this.http.get<{ data: DispatchStats }>(`${this.base}/org/${orgId}/dispatch/stats`);
  }
  ...
  createDispatch(orgId: string, body: object) {
    return this.http.post<{ data: DispatchItem }>(`${this.base}/org/${orgId}/delivery`, body);
  }

  updateDispatch(orgId: string, id: string, body: object) {
    return this.http.patch<{ data: DispatchItem }>(`${this.base}/org/${orgId}/dispatch/${id}`, body);
  }
}
